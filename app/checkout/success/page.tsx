...
export default function SuccessPage() {
  ...
  const payment_intent = sp.get("payment_intent"); // ✅ correct in client
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      if (stopped) return;
      // blijf pollen zolang het pending is
  ...
  return (
    <div className="max-w-2xl mx-auto py-24 px-6">
      ...
      {!order && !error && (
        <p className="opacity-80">Payment verwerken… even wachten.</p>
      )}
      ...
      {order && (
        <div className="mt-6 space-y-3">
          <div className="bg-black border border-gray-700 rounded-xl p-4">
            <p>
              <span className="opacity-70">Order:</span> #{order.id}
            </p>
            <p>
              <span className="opacity-70">Status:</span> <b>{order.status}</b>
            </p>
            <p>
              <span className="opacity-70">Totaal:</span> €
              {Number(order.total).toFixed(2)}
            </p>

            {order.status === "paid" && (
              <p className="text-green-400 mt-2">
                ✅ Betaald — winkelwagen is geleegd.
              </p>
            )}
            {order.status === "pending" && (
              <p className="opacity-80 mt-2">
                ⏳ Wachten op bevestiging (webhook)…
              </p>
            )}
          </div>
